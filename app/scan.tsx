...
export default function ScanTimesheet() {
 ...
 return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.container}>
  <AnimatedPressable onPress={() => router.back()}><Text style={styles.back}>‹ Back</Text></AnimatedPressable>
...
function ScanTimesheetPreview(){
 ...
 return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.container}>
  <AnimatedPressable onPress={()=>router.back()}><Text style={styles.back}>‹ Back</Text></AnimatedPressable><Text style={styles.header}>Scan Timesheet</Text><Text style={styles.subtitle}>Choose an image, then review every detected row before importing. Nothing is auto-saved.</Text>
  <View style={styles.actions}>{Platform.OS!=="web"&&<AnimatedPressable style={styles.primary} onPress={()=>void choose(true)}><Text style={styles.primaryText}>TAKE PHOTO</Text></AnimatedPressable>}<AnimatedPressable style={styles.secondary} onPress={()=>void choose(false)}><Text style={styles.secondaryText}>{Platform.OS==="web"?"UPLOAD IMAGE":"CHOOSE PHOTO"}</Text></AnimatedPressable></View>
  ...
  {!!rows.length&&<><Text style={styles.review}>Review Imported Entries</Text>{rows.map(row=><View key={row.id} style={styles.card}>
   <AnimatedPressable onPress={()=>setRows(old=>old.map(r=>r.id===row.id?{...r,selected:!r.selected}:r))}><Text style={styles.select}>{row.selected?"☑ Selected":"☐ Not selected"}</Text></AnimatedPressable>
  ...
  </View>)}<AnimatedPressable style={styles.primary} onPress={()=>void importRows()}><Text style={styles.primaryText}>IMPORT REVIEWED ENTRIES</Text></AnimatedPressable></>}
 </ScrollView></SafeAreaView>;
}
